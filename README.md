...
| `research-plan-annotate` | 実装前に「調べる → 計画する → 注釈で磨く」を回し、`research.md` / `plan.md` を成果物とする | `/research-plan-annotate` | superpowers の `executing-plans` または `subagent-driven-development` へ引き継ぎ |
...
全フェーズ完了後、superpowers の `executing-plans` または `subagent-driven-development` に実装を引き継ぎます。
