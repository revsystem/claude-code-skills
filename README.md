...
| `research-plan-annotate` | 実装前に「調べる → 計画する → 注釈で磨く」を回し、`research.md` / `plan.md` を成果物とする | `/research-plan-annotate` | [superpowers](https://github.com/obra/superpowers) の `executing-plans` または `subagent-driven-development` へ引き継ぎ |
...
### npx skills でインストール（スキルのみ）

[npx skills](https://github.com/vercel-labs/skills) CLI を使うと、スキルだけを手軽にインストールできます。

```bash
npx skills add revsystem/claude-code-skills
```

個別のスキルを指定する場合:

```bash
npx skills add revsystem/claude-code-skills -s handover
```

グローバルインストール（全プロジェクトで利用）:

```bash
npx skills add revsystem/claude-code-skills -g
```
...
エージェントとフックは `npx skills` の管理対象外です。これらも含めた完全なセットアップには、以下の `install.sh` を使用してください。
...
~/.claude/skills/research-plan-annotate  →  {REPO}/skills/research-plan-annotate/
...
全フェーズ完了後、[superpowers](https://github.com/obra/superpowers) の `executing-plans` または `subagent-driven-development` に実装を引き継ぎます。
...
`skills/{skill-name}/SKILL.md` や `agents/{agent-name}.md` を編集すると、シンボリックリンク経由で即時反映されます。新しいスキル・エージェントを追加した後は `./install.sh` を再実行してリンクを作成してください。
