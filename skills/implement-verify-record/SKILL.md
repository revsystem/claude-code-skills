...
深度は plan.md ヘッダの「深度」フィールドから読み取る（research-plan-annotate が計画時に宣言した minimal / standard / comprehensive）。フィールドが無い場合は standard を既定とし、その旨を着手前のゲートで明示する。深度がベースラインを決める。
...
- auto/bypass・無人実行とは非互換。ゲートはツール承認の自動化とは別レイヤの人手の関門であり、auto の非中断圧力より優先する。承認できる人間がいなければゲートを越えず停止する。
