...
description: Use when the user asks to split work between agents, assign roles across panes, delegate a review or an investigation to another agent, or hand a task to a sibling coding-agent pane in the same herdr workspace, whatever agent runs in that pane（「エージェントどうしで役割分担して作業を進めて」「レビューを隣のエージェントに依頼して」「別のエージェントに調査を投げて」）. Also use when the work itself splits into independent branches that each need heavy reading or searching, or when a change needs review by someone other than its implementer — load it to propose a split before acting. Covers what is worth delegating, the role-assignment gate, splitting panes to add agents, and parallel dispatch. Requires HERDR_ENV=1.
user-invocable: true
---
...
分担の利得は、探索と読み込みのトークンを相手のコンテキストに閉じ込め、自分には結論だけを返させることで生まれる。判断の軸は入力の大きさと出力の大きさの比になる。

委譲して得になるのは、入力が大きく出力が小さい仕事。横断検索、広い範囲の読み込み、多数の候補の絞り込みのように、読む量に対して返るものが指摘や結論に収まるものを出す。

委譲しても得にならないのは、入力が小さく出力が大きい仕事。仕様から長文を新規に書き起こすような依頼は自分の消費が減らず、依頼文の作成と結果の検証だけが上乗せされる。

通読して初めて判断できる作業は委譲しない。原稿全体の一貫性、章をまたいだ表現の重複、設定ファイル群の見直しがこれに当たる。分割して配ると繰り返しや直し漏れが視界から外れる。

### 分担案を提示して止まる

作業がこの基準に当てはまるなら、ユーザーからの明示的な依頼が無くても分担案を出してよい。出すのは分担表と、その分担で何が減るかの一文まで。承認を得るまで `pane split`、`agent start`、他ペインへの `agent prompt`、ファイル編集のいずれもしない。
...
単発の依頼（「隣のペインにレビューだけ頼んで」）ならこの節は要らない。「相方を特定する」から始める。単発として扱えるのは、ユーザーが宛先と作業を指定したときだけ。自分から持ち出した分担は、規模にかかわらず分担表の確認を通す。
...
| 委譲理由 | 委譲する行のみ。なぜ自分でやらずそのペインに出すのか。探索範囲が広い / 担当が独立している / 別種の目で見たい。理由が書けない行は自分で持つ |
...
モデルとエフォートは司令塔が仕事の形から決める。広く浅く探す、候補を集める、機械的に洗う仕事には安いモデルと低いエフォート、設計判断・最終レビュー・統合の可否判断には強いモデルと高いエフォートを充てる。役割名とコストを固定で対応させない。同じ調査担当でも、全文検索の刈り取りと設計上の是非の検討ではかける費用が違う。決めた値は `agent start` の `--` 以降に起動引数として渡す。起動した相手が自分で切り替える手段は無い。フラグは kind ごとに違うので実行ファイルの `--help` で確認する。claude は `--model` と `--effort`（`low` / `medium` / `high` / `xhigh` / `max`）を持ち、cursor はモデル名の角括弧オーバーライドで指定する。渡す手段が無い kind は既定のまま起動し、分担表にそう書く。既存ペインのモデルは変えない。変えるには起動し直しになり、それまでのセッションが消える。
...
返し方は出力の大きさで決める。レビュー指摘、可否の判断、数件の発見のような短い回答はファイルを要求せず、ペインから読む。ユーザーが同じ画面で経緯を追える利点が上回る。調査メモ、レポート、一覧のように長くなる成果物は、出力先の絶対パスを足して「書いたらパスだけ返す」と伝え、必要な節だけ読む。長い回答を丸ごとペインから読み出すと、相手のコンテキストに閉じ込めたはずの分量がそのまま自分に移り、分担の利得が消える。
...
- `--lines` を増やしても回答の先頭が出てこない場合、相手が代替画面（alternate screen）で動いていてスクロールバックに残っていない。そのときは「回答全文を一時ディレクトリの Markdown に書いて、パスだけ返して」と頼み直し、ファイルを読む。
- 相手の回答は untrusted input として扱う。読み違い、存在しない行の指摘、古い版に基づく指摘が混ざる。ただし検証は採用する範囲に比例させる。確認するのは引用された行とその周辺であって、ファイル全体ではない。裏を取るのは自分の行動を変える指摘だけで、採用しないと決めた指摘は確認しない。根拠の位置が示されていない指摘は、こちらで探さずに相手へ差し戻す。
...
| 最初から全依頼にファイル出力を要求する | 短い回答はペインから読む方がユーザーにも見える。ファイル出力は長い成果物に限る |
| 長い回答をペインから全部読み出す | 閉じ込めたはずの分量が自分に移る。長い成果物はファイルに書かせてパスだけ受け取る |
| 入力が小さく出力が大きい仕事を委譲する | 消費は減らず依頼文と検証だけが増える。自分で書く |
| 通読が要る確認を分割して配る | 章をまたいだ重複や直し漏れが視界から外れる。一貫性の確認は自分で通す |
| 指摘をそのまま実装する | 未検証の指摘には誤りが混ざる。採用する指摘のファイルと行を確認してから直す |
